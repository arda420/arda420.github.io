...
const ipgeolocation = 'https://api.ipgeolocation.io/ipgeo?apiKey=1b69fde6fce94c708e3228079dd6b1b6';
...
$(document).ready(() => {
  ...
  for (let i in links) {
    ...
    if (i != links.length - 1) $('#marquee').append(' <img class="emoticon" src="mgh_17.png"> ');
  }

  if (mobileAndTabletCheck()) {
    $('#background').replaceWith('<div id="background" style="background-image: url(mobile-background.jpg);"></div>');
...
$.getJSON(ipgeolocation, (data) => {
  writeLine(['Authenticating...', "Granting access to <span style='font-size: 14px; color: #06d;'>[unknown]</span>..."], 30, () => {
    ...
    writeLine([`Access granted! <span style='font-size: 14px; color: #0f0;'>[success]</span>`, `Welcome back, <i style='color: #0f0'>${ip}</i>! By the way, nice to see someone from ${country} here!`], 30, 500, () => {
      ...
      writeLine([`<i style='color: #F62459'>arda#0420 $$$</i>`], 120, 500, () => {
        timeouts.push(
          setTimeout(() => {
            if (app.skippedIntro) return;
